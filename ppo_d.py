...
class Agent():
    ...
    def __init__(self):
        ...
        self.buffer = []
        self.optimizer_a = optim.Adam(self.anet.parameters(), lr=3e-4)
        self.optimizer_c = optim.Adam(self.cnet.parameters(), lr=1e-3)
    ...
    def update(self):
        ...
        for i in range(self.ppo_epoch):
            action_probs = self.anet(s).gather(1, a)
...
for i_ep in range(1000):
    ...
    for t in range(200):
        ...
        if args.render:
            env.render()
        agent.buffer.append(Transition(state, action_index, action_prob, (reward + 8) / 8, state_))
        score += reward
        if (t + 1) % 32 == 0:
            agent.update()
        state = state_
